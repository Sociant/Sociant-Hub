...
import {
	Container,
	Loader, MotionError404, MotionLink, MotionLoader, MotionReturn, SpinnerButton,
} from '../styledComponents/globalStyles'
import { useApp } from '../provider/AppProvider'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faBoxOpen, faChevronLeft, faCircleNotch } from '@fortawesome/pro-light-svg-icons'
import { useTranslation } from 'react-i18next'
import { ActivityEntry, ErrorResponse, HomeResponse, StatisticsResponse, TwitterUserExtended } from '../types/global'
...
import {
	formatActionExpanded, formatDate,
	getAccountAge,
	getActionIcon,
	getDifference,
	thousandSeparator,
...
export default function User() {
	...
	if(twitterUser == null) return <MotionError404
			...
			<MotionUserCard
				...
				<motion.div
					...
					<Timeline>
						...
						{ activities.map((item: ActivityEntry, index) => <div className="item" key={ `${index}-${item.uuid}` }>
							<FontAwesomeIcon icon={ getActionIcon(item.action) } color={
						...
						{ activities.length === 0 && <div className="empty">
					...
					<UserData>
						...
						<div className="profile-item">
							...
							<div>
								...
								<TwitterButtonStyles target='_blank' href={ `https://twitter.com/${ twitterUser.screen_name }` }>
						...
						<div className="item-row">
							<div className="item">{ thousandSeparator(twitterUser.followers_count) }<small>{ t('profile.userAnalytics.followers') }</small></div>
							<div className="item">{ thousandSeparator(twitterUser.friends_count) }<small>{ t('profile.userAnalytics.following') }</small></div>
						</div>
						<div className="item-row">
							<div className="item">{ thousandSeparator(twitterUser.statuses_count) }<small>{ t('profile.userAnalytics.tweets') }</small></div>
							<div className="item">{ thousandSeparator(twitterUser.listed_count) }<small>{ t('profile.userAnalytics.listedIn') }</small></div>
							<div className="item">{ thousandSeparator(twitterUser.favorites_count) }<small>{ t('profile.userAnalytics.liked') }</small></div>
						</div>
						<div className="item smaller">{ accountAge }<small>{ t('profile.userAnalytics.age') }</small></div>
					</UserData>
				</motion.div>
			</MotionUserCard>
		</UserPage>
